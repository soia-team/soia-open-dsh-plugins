...
## 2026-09-22 · 面板自检（运行状况）与它抓出的三处缺陷

### 为什么加

面板原来只报告会话在做什么，不报告**自己还准不准**："会话安静"和"这个视图收不到数据了"在屏幕上一模一样。这不是仪表，是装饰。

第五个模块「运行状况」把折叠自身的数字摆出来：已折叠事件 / 已忽略（会话管理类）/ 未知类型 / 收到流式帧 / 已接管 agent 与注册表可见 / 增量接受与丢弃 / 数据新鲜度（运行中静默超 60 秒标红）。

### 建成后立刻抓出的三处（全部由新数字发现，不靠猜）

| 缺陷 | 证据 | 处置 |
| --- | --- | --- |
| 失败被记成成功 | 日志出现 `Error: grep search failed` 而概览「失败 0」 | 宿主 `isError` 未传到动作记录 + 工具在成功返回里自报的 `{"status":"error"}` 未读；已修并复验（失败 2、两行标红、筛选精确） |
| 「未知类型」无意义 | 一次短会话 11 条"未知"，实为 `session`/`request/header`/`permission/preset` 等会话管理类 | 抽出 `IGNORED_TYPES` 白名单；此后 `unknown` 才表示"本构建没见过的类型" |
| 流式帧通道从未生效 | 「收到流式帧 0」「已接管 agent 0」 | `agent/assistant-stream` 用 agent 作用域派发，根 ctx 收不到；改为在 `session/event` 上懒挂到 `agents` 注册表里的 agent。**实测 `agents.list()` 在本地 Web profile 下始终为空**（哨兵值排除了"服务不可达/查询抛错"两种可能），因此如实登记为已知限制：面板可见内容不依赖该通道，计数器把这条限制显示在界面上 |

### 设计取舍（未做）

右侧栏常驻面板（参考插件的 `sidebar.right.pane.tab`）**不在 v1**：页签已集成并验证，再加一个面板是新的集成面（另一套插槽契约与道具），换来的能力重复。v1 的原则是少一个未验证的入口比多一个好。

## 2026-09-22 · 任务面板的模块架构与失败语义

### 架构（按参考插件的信息组织方式重排）

面板从"一张表 + 一个列表"改为**四模块**，模块边界即设计：**概览**（卡片网格：状态/位置/工具调用/失败，失败标红）、**正在跑**（在飞调用：工具 + 参数 + 已跑）、**动作日志**（五列：时间·工具·干了什么·耗时·结果，带「全部 N / 只看失败 N」筛选，失败行标红）、**最近动静**（人话短语）。每个模块自带文案键前缀（`overview.*`/`running.*`/`log.*`/`recent.*`）与空态，可独立阅读、翻译、移除。

### 活会话抓到的第三个真缺陷：失败被记成成功

| 现象 | 根因 | 修法 |
| --- | --- | --- |
| 日志里 `Error: grep search failed (exit 2)` 显示「完成」，概览「失败 0」 | 宿主层 `isError` 只写进 `lastTool`，**没传给动作记录**（`actionOf` 读的是过滤前那份未标失败的副本） | 显式把失败标记传进动作记录 |
| 强制失败的调用（不可达页面、不存在的文件）仍显示「完成」 | 本生态工具把失败写在**成功返回里**（`{"status":"error","code":…}`），`isError` 为 false，折叠完全没读这一层 | 新增 `toolResultFailed`：同时认宿主 `isError`、顶层 `error`、与结果首行 JSON 的 `status ∈ {error, failed, not_found, unavailable, denied, timeout, invalid}` |

**活会话验证**：三次调用（两次强制失败）→ 概览「工具调用 3 / 失败 2」，两行标红，`只看失败 2` 精确列出 `status=error, code=not_found` 与 `navigation_failed`。

**新增测试**：失败传播 2 例 + 工具自报失败 2 例（包内 77 用例，仓总 393+）。

## 2026-09-22 · 规则语料体检（六条规则 × 46 条真实命令）与面板第三版

### 规则语料体检

活会话验收暴露两条误伤之后，把六条规则对着 46 条真实命令过了一遍。规则引擎是纯函数，所以这一遍做成了**表驱动测试**（`packages/safe-tool-call-policy/tests/shared/rule-corpus.test.ts`），不是靠模型跑：每条都声明"该命中谁"或"谁都不该命中"，正反两侧都钉住。

又抓出 3 条（累计 5 条）：

| 类型 | 命令 | 原因 | 修法 |
| --- | --- | --- | --- |
| 漏报 | `sqlite3 ~/.myapp/data/platform.db "delete from t"` | R1 只认 `rm/mv/cp/…` 与重定向，不认数据库客户端对数据根执行 DML | 补 DB 客户端 + DML 关键字分支 |
| 误报 | `curl -H "Authorization: Bearer $API_TOKEN"` | R2 只看头名，值来自环境变量也照样命中 | 该分支要求值整体不含 `$` |
| 抢判 | `git worktree remove --force …` | R5 与 R6 有同一分支，先出现的 R5 抢走判定 | 删掉 R5 的重复分支，worktree 由 R6 专管 |

同一次体检还纠正了我自己的一条错判：`git commit -m "…"` 被当成"词面误报"，其实它本来就该被 R4 拦（未用 `--only`）。语料已改成真正测词面免疫的 `echo "the password field needs validation"`。

### 面板第三版：从"协议词"到"人话日志"

第二版把 `step/start`、`session-log-*/delivery-accepted` 这类**协议事件名**直接显示给用户，被判定为不可读。第三版：

- 四列动作日志：**时间 · 工具 · 干了什么 · 结果**，结果列带 `完成/失败 · N 秒`；
- 参数摘要取最多两个有意义字段（`#card @ http://…`），JSON 结果折成 `key=value` 短句；
- 无法识别的与传输记账类事件**不再显示**为"其他活动"；
- 两个列表都有上限（8 次调用 / 6 条事件），线包不随回合长度增长；
- 文案中英双语，键集由 `Record<LiveTaskKey, string>` 钉住；工具名故意不翻译。

**新增测试**：可读性 3 例（选择器 @ 页面、JSON 结果折叠、散文结果原样）。仓总 **393 用例**。

**顺带修掉的两个自伤**：文档注释里写 `session-log-*/delivery-accepted`，其中 `*/` 提前关闭块注释导致整段文档被当代码编译；README 仍写着"`lib/client.js` 不存在、从未渲染"——中英两侧已按现状重写并重算 i18n 哈希。
