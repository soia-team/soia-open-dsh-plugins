...
Fixed. While the tool is visible, its name, description and four-parameter schema enter every assembly. **Measured: the model-visible projection (the JSON of `name` + `description` + `parameters`) is 574 characters, about 144 tokens at the host token-meter's fixed density (≈4 characters per token); `pnpm run check-token-budget` recomputes it from the built artifact and compares it with `dsh.tokenBudget.resident` in `package.json` (179 = 144 + 35 for the section), failing CI when exceeded.** The earlier hand estimate of 214 tokens used a wider basis (the whole registered definition) and is superseded. The package appends no retained or dynamic context. A scope that hides the tool removes this contribution entirely.

2026-09-21 slimming record: the description went from 257 to 173 characters (the error-code sentence was dropped — a failed call already returns `status: "error"` with a `code`), and the four parameter descriptions went from 182 to 94 characters in total; tool block 198 → 144, prompt section 40 → 35, resident 238 → 179.
...
- **Compatibility is unverified.** The `dsh.compatibility.dsh` range `>=0.1.0-rc.8 <0.2.0` follows ecosystem convention; under strict node-semver semantics that range does **not** match prereleases (such as `0.1.5-rc.2` or `0.1.6-alpha.2`), because a prerelease needs a comparator on the same tuple. The peer dependencies therefore enumerate the published prereleases one by one. This package has been verified loading on **DSH `0.1.6-alpha.2`** (see the next bullet); the `dshReleases` map follows once other versions have evidence.
- **Verification depth.** The repository tests cover the measurement core, including real-browser cases. **Loading has been observed**: a git-source install into a throwaway profile under a separate `DSH_HOME` reported `fiberPhase: active` in `pluginInventory/list`. **A real model-initiated call has been observed (2026-09-21)**: the model chose `check_ui_size` on its own, passed the three parameters correctly, and read 34px with `diff.height = 7` (with the pre-slimming description; not re-run after slimming). The evidence per step is in [docs/verification.md](../../docs/verification.md).
