...
import { UiSizeHealth, UiSize_HEALTH_SERVICE } from '../../src/host/health.ts'
...
describe('runtime self-check', () => {
  it('publishes a service under a stable name', () => {
    ...
    new UiSizeHealth(ctx as never)

    expect(UiSize_HEALTH_SERVICE).toBe('checkUiSizeHealth')
    expect(ctx['checkUiSizeHealth']).toBeDefined()
  })

  it('counts calls and failures, and reports nothing before the first call', () => {
    const health = new UiSizeHealth(fakeContext() as never)
