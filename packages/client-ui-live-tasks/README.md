...
| 区域 | 内容 |
|---|---|
| 顶部一行 | 状态点 + 「本会话在做什么」+ 阶段（`正在干活` / `等工具返回` / `已结束` / `空闲`）+ 回合与步骤 + 已运行秒数 |
| 正在跑 | 每个在飞的调用一行：状态点 + **工具名** + **参数摘要**（命令 / 路径 / `选择器 @ 页面`）+ 已跑秒数 |
| 干过什么 | 四列表格：**时间**（本地 `HH:MM:SS`）、**工具**、**干了什么**、**结果**（返回首行摘要 + `完成/失败 · N 秒`），保留最近 8 次 |
