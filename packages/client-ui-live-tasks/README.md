...
本包把 `session/event` 与 `agent/assistant-stream` 折成一个小状态对象（`LiveTaskState`），在会话视图区注册一个只读页签「活动」——与内置的「对话 / 轨迹」同排（`conversation.view`，`order: 20`）。页签名取「活动」而不是「任务」：待办类插件已经占用了「任务」，而这里是实时活动时间线。
...
- **「可用工具」数常常显示 `—`。** 它取自请求头里的工具清单，而该事件在会话事件流里未必可见（本机实测为不可见），所以宁可显示未知也不猜。真实可用的替代指标是同一行的「用到 N 种」，它来自实际发生的调用。
