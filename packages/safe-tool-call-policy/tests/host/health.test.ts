...
import { PolicyHealth, Policy_HEALTH_SERVICE } from '../../src/host/health.ts'
...
describe('runtime self-check', () => {
  it('publishes a service under a stable name', () => {
    ...
    new PolicyHealth(ctx as never)

    expect(Policy_HEALTH_SERVICE).toBe('safeToolCallPolicyHealth')
    expect(ctx['safeToolCallPolicyHealth']).toBeDefined()
  })

  it('counts calls and failures, and reports nothing before the first call', () => {
    const health = new PolicyHealth(fakeContext() as never)
