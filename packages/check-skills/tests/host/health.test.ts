...
import { SkillsHealth, Skills_HEALTH_SERVICE } from '../../src/host/health.ts'
...
describe('runtime self-check', () => {
  it('publishes a service under a stable name', () => {
    ...
    new SkillsHealth(ctx as never)

    expect(Skills_HEALTH_SERVICE).toBe('checkSkillsHealth')
    expect(ctx['checkSkillsHealth']).toBeDefined()
  })

  it('counts calls and failures, and reports nothing before the first call', () => {
    const health = new SkillsHealth(fakeContext() as never)
