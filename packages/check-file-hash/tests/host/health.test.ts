...
import { FileHashHealth, FileHash_HEALTH_SERVICE } from '../../src/host/health.ts'
...
describe('runtime self-check', () => {
  it('publishes a service under a stable name', () => {
    ...
    new FileHashHealth(ctx as never)

    expect(FileHash_HEALTH_SERVICE).toBe('checkFileHashHealth')
    expect(ctx['checkFileHashHealth']).toBeDefined()
  })

  it('counts calls and failures, and reports nothing before the first call', () => {
    const health = new FileHashHealth(fakeContext() as never)
